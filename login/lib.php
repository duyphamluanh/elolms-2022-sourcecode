...
function core_login_process_password_reset($username, $email) {
    ...
    //$url = $CFG->wwwroot.'/index.php'; Origin
    $url = 'https://mail.google.com'; //Nhien elo link toi trang gmail
...
function core_login_process_password_set($token) {
    ...
    if (empty($data)) {
        ...
        echo $OUTPUT->header();
        //echo $OUTPUT->box(get_string('setpasswordinstructions'), 'generalbox boxwidthnormal boxaligncenter'); // Nhien elo an lang dat mat khau
        $mform->display();
        echo $OUTPUT->footer();
        return;
    } else {
