...
/*nhien elo active tab*/
define('TAB_SUMMARY', 0);
/**
 * The home page should be the users my page
 */
define('TAB_CONTENT', 1);
/**
 * The home page can be chosen by the user
 */
define('TAB_LEARNINGSCHEDULE', 2);
/*nhien elo end active tab*/
/**
 ...
 * @return string
 */
//nhien elo 15_01_2019 ham get gradetype @return true/false
/*function get_eloprogress_gradetype($course,$activity) {
    global $DB;
    $return = true;
    $graderecset = $DB->get_recordset_select('grade_items', 'courseid = ' . $course->id . 
                        ' AND iteminstance = '. $activity->instance , null,'','gradetype,hidden');
    foreach ($graderecset as $graderec) {
        if ($graderec->gradetype == 0) {
            $return = false;
            break;
        }*/
        /*if ($graderec->hidden == 1) {
            $return = false;
            break;
        }*/
   // }
...
function authenticate_user_login($username, $password, $ignorelockout=false, &$failurereason=null) {
    ...
    if ($user = get_complete_user_data('username', $username, $CFG->mnet_localhost_id)) { // Nhien replaced
...
// Nhien Login : if khong co trong danh da cho phep thi mac dinh cho phep manual  
        else{
            $auth = 'manual';
            $user->auth = $auth;
            if (in_array($user->auth, $authsenabled)) {
