...
class cm_info implements IteratorAggregate {
    ...
    /**
     * @var nhien allow_prerequisiton_event
     */
    public $elo_allow_prerequisiton_event;
    /**
     * List of class read-only properties and their getter methods.
     * Used by magic functions __get(), __isset(), __empty()
     * @var array
    ...
    private function obtain_dynamic_data() {
        ...
        if ($this->state >= self::STATE_BUILDING_DYNAMIC || $userid == -1) {
            // Nhien allow_prerequisiton_event
            if($this->elo_allow_prerequisiton_event == 1) { // // Nhien Kiem Tra uservisbe cua event tien quyet;
               $nhienhas_capability = has_capability('moodle/course:viewhiddenactivities', $this->get_context(), $userid);
               $nhienhas_get_available = $this->available;
               $nhienhas_capability2 = has_capability('moodle/course:ignoreavailabilityrestrictions', $this->get_context(), $userid);
               if($this->visible && !$nhienhas_capability && !$nhienhas_capability2 && !$nhienhas_get_available && $this->visibleoncoursepage){
                   $this->uservisible = true;
               }
            }
            // End Nhien allow_prerequisiton_event
           return;
        }        
    ...
    private function update_user_visible() {
        ...
        // Nhien allow_prerequisiton_event
        if($this->elo_allow_prerequisiton_event == 1){
            $nhienhas_capability = has_capability('moodle/course:viewhiddenactivities', $this->get_context(), $userid);
            $nhienhas_get_available = $this->get_available();
            $nhienhas_capability2 = has_capability('moodle/course:ignoreavailabilityrestrictions', $this->get_context(), $userid);
            if($this->visible && !$nhienhas_capability && !$nhienhas_capability2 && !$nhienhas_get_available && $this->visibleoncoursepage){
                $this->uservisible = true;
            }
