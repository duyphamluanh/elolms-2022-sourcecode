        ...
        if (empty(self::$eventfactory)) {
            ...
            self::$eventfactory = new event_factory(
                ...
                function ($dbrow) {
                    ...
                    $cm->elo_allow_prerequisiton_event = $dbrow->elo_allow_prerequisiton_event; // Nhien allow_prerequisiton_event
                    if (!$cm->uservisible) {
                        return true;
                    }
                    $dbrow->elo_noncomplete_prerequisiton_event = ($cm->uservisible && !$cm->available);// End Nhien allow_prerequisiton_event
