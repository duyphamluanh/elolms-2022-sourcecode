 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_transfer_course_renderer extends plugin_renderer_base {
     ...
     */
     ...
     public function ou_get_management_viewmodes() {
        return array(
    ...
    public function management_heading($heading, $viewmode = null, $categoryid = null) {
        ...
        if ($viewmode !== null) {
            ...
            $html .= $this->view_mode_selector($this->ou_get_management_viewmodes(), $viewmode);
            if ($viewmode === 'courses') {
...
//        $listing = core_course_category::top()->get_children();
    ...
    public function category_listitem(core_course_category $category, array $subcategories, $totalsubcategories,
        ...
        $viewcaturl = new moodle_url('/blocks/transfer_course/index.php', array('categoryid' => $category->id));
        if ($isexpanded) {
...
//        $html .= $this->course_bulk_actions($category);
    ...
    protected function listing_pagination(coursecat $category, $page, $perpage, $showtotals = false,
        ...
        if ($viewmode !== 'default') {
            $baseurl = new moodle_url('/blocks/transfer_course/index.php', array('categoryid' => $category->id,
                'view' => $viewmode));
        } else {
            $baseurl = new moodle_url('/blocks/transfer_course/index.php', array('categoryid' => $category->id));
        }
    ...
    public function course_listing_actions(coursecat $category, course_in_list $course = null, $perpage = 20) {
        ...
        if ($category->can_resort_courses()) {
            ...
            $baseurl = new moodle_url('/blocks/transfer_course/index.php', $params);
        ...
        $menu = new action_menu(array(
                new action_menu_link_secondary(new moodle_url($this->page->url, array('perpage' => 25)), null, 25),
     ...
     */
    //nhien
        public function outech_get_course_listitem_actions(\coursecat $category, \course_in_list $course) {
        $baseurl = new \moodle_url(
            '/blocks/transfer_course/index.php',
        ...
        if ($course->can_edit()) {
            $actions[] = array(
                'url' => new \moodle_url('/blocks/transfer_course/edit.php', array('id' => $course->id , 'returnto' => 'catmanage')),
...
//        $actions = \core_course\management\helper::get_course_listitem_actions($category, $course);
        $actions = $this->outech_get_course_listitem_actions($category, $course);
        if (empty($actions)) {
            return '';
        }
        $actionshtml = array();
    ...
    public function course_detail(course_in_list $course) {
        ...
        $html .= html_writer::end_div();
        $html .= html_writer::start_div('card-body');
//        $html .= $this->course_detail_actions($course); // unnecessary 
        ...
        if ($course->can_access()) {
            if ($course->can_edit()) {
                $actions[] = $this->output->action_icon(
                    new moodle_url('/blocks/transfer_course/edit.php', array('id' => $course->id)),
     ...
     * @return string
     */
    public function elo_transfer_course_search_form($value = '', $format = 'plain') {
        static $count = 0;
        $formid = 'coursesearch';
        if ((++$count) > 1) {
            $formid .= $count;
        ...
        $searchurl = new moodle_url('/blocks/transfer_course/index.php');
        ...
        $output .= html_writer::tag('div', $this->output->heading($strsearchcourses.': ', 3, 'm-0'),
                array('class' => 'card-header'));
        $output .= html_writer::start_div('card-body');
        $output .= html_writer::start_div('input-group col-sm-5 col-lg-3 m-auto');
