...
class event_vault implements event_vault_interface {
    ...
    // Nhien calendar elocolor        
        $elocoursindexs = array();
        $bangmautot = array(3,35,14,19,27,32,39,42,52,53);
        $bangmau = array(// 54 
                                '#05fec1',
                                '#32af87',
                                '#387261',
        ...
        while ($records = array_values($this->retrievalstrategy->get_raw_events(
            ...
            foreach ($records as $record) {
                $record->elo_allow_prerequisiton_event = 1;// Nhien allow_prerequisiton_event
                if ($event = $this->transform_from_database_record($record)) {
                    $filtertest = $filter ? $filter($event) : true;

                    if ($event && $filtertest){   
                        // Nhien Set Mau;
                        $chocourse = $event->get_course();
                        $elocourse = $chocourse->get('id');
                        if(!empty($elocourse)){
                            if($elocoursindexs[$elocourse] == null){
                                $colorindex = count($elocoursindexs);
                                if($colorindex<10){
                                    $elocoursindexs[$elocourse] = $bangmau[$bangmautot[$colorindex]-1];
                                }
                                else {
                                    $elocoursindexs[$elocourse] = $bangmau[$colorindex%54];
                                }
                            }
                            $event->elocolor = $elocoursindexs[$elocourse];
                            if($record->elo_noncomplete_prerequisiton_event==true){
                                $event->elocolor = '#aaaaaa';
                            }
                        }
                        $events[] = $event;
                    }
