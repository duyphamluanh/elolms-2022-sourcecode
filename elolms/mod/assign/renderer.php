...
class mod_assign_renderer extends plugin_renderer_base {
    ...
    public function render_assign_files(assign_files $tree) {
        ...
        $html .= '</div>';
        if (file_exists($CFG->dirroot . '/local/assignpdf/lib.php')) {
            require_once($CFG->dirroot . '/local/assignpdf/lib.php');
            $action = trim(optional_param('info', '', PARAM_TEXT));
            if (isset($action) && $action === 'core_get_fragment') {
                $this->renderid = html_writer::random_id('assign_files_render');
                $html .= '<div id="' . $this->renderid . '">';
                $html .= custom_htmllize_render($tree->dir, $this->page->context->id);
                $html .= '</div>';
            }
        }
