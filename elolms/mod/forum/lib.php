...
WHERE
    ...
    if ($forum->completiondiscussions) {
        $value = $forum->completiondiscussions <= $DB->count_records('forum_discussions', array('forum' => $forum->id, 'userid' => $userid));
    ...
    if ($forum->completionreplies) {
        $value = $forum->completionreplies <= $DB->get_field_sql($postcountsql . ' AND fp.parent<>0', $postcountparams);
        if ($type == COMPLETION_AND) {
            $result = $result && $value;
        } else {
            $result = $result || $value;
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                $data = new \mod_forum\output\forum_post_email(
                        $course, $cm, $forum, $discussion, $post, $userfrom, $userto, $canreply
                );
...
function forum_search_posts($searchterms, $courseid = 0, $limitfrom = 0, $limitnum = 50, &$totalcount, $extrasql = '') {
    ...
    if ($lexer->parse($searchstring)) {
        ...
        list($messagesearch, $msparams) = search_generate_SQL($parsearray, 'p.message', 'p.subject', 'p.userid', 'u.id', 'u.firstname', 'u.lastname', 'p.modified', 'd.forum', $tagfields);
...
function forum_update_post($newpost, $mform, $unused = null) {
    ...
    $post->message = file_save_draft_area_files($newpost->itemid, $context->id, 'mod_forum', 'post', $post->id, mod_forum_post_form::editor_options($context, $post->id), $post->message);
    $DB->update_record('forum_posts', $post);
    $DB->update_record('forum_discussions', $discussion);
...
// $forum is an object
    ...
    //toan 08112021 elolms-2021-khongchophepsvdangbaipost-08
    if ($forum->assesstimestart > 0 && $forum->assesstimefinish > 0) {
...
function forum_user_can_see_post($forum, $discussion, $post, $user = null, $cm = null, $checkdeleted = true) {
    ...
    if ($forum->type == 'qanda') {
        if (has_capability('mod/forum:viewqandawithoutposting', $modcontext, $user->id) || $post->userid == $user->id || (isset($discussion->firstpost) && $discussion->firstpost == $post->id)) {
            return true;
        }
...
// Nhien Check Forum is grading
    ...
    foreach ($discussions as $discussion) {
        ...
        if ($eloforumgrading)
            $elosumelorates += $discussion->elorates;
    ...
    switch ($displayformat) {
        case 'header':
            ...
            if ($eloforumgrading)
                echo '<td class="sumelorates"><span>' . $elosumelorates . '</span></td>';
...
function forum_get_context($forumid, $context = null) {
    ...
    if (!$context || !($context instanceof context_module)) {
        ...
        if ($PAGE->cm && $PAGE->cm->modname === 'forum' && $PAGE->cm->instance == $forumid && $PAGE->context->contextlevel == CONTEXT_MODULE && $PAGE->context->instanceid == $PAGE->cm->id) {
...
function mod_forum_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $node = new core_user\output\myprofile\node('miscellaneous', 'forumdiscussions', $string, null, $discussionssurl);
    $tree->add_node($node);
...
function mod_forum_core_calendar_provide_event_action(calendar_event $event, \core_calendar\action_factory $factory) {
    ...
    return $factory->create_instance(
                    get_string('view'), new \moodle_url('/mod/forum/view.php', ['id' => $cm->id]), $itemcount, true
    );
}
