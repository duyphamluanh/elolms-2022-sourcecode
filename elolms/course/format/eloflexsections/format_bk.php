        ...
        foreach($activities as $activity) {
            if ($activity->modname == 'quiz' || $activity->modname == 'assign' || $activity->modname == 'forum') {
                ...
                if ($activity->completionexpected) {
                    ...
                    $enddatetext=userdate($course->enddate,get_string('strftimerecent'));
                ...
                if($assigntimetext->endtimetext){
                    ...
                    $enddatetext = userdate($assigntimetext->endtimetext,get_string('strftimerecent'));
...
if (($deletesection = optional_param('deletesection', 0, PARAM_INT)) && confirm_sesskey()) {
    $renderer->confirm_delete_section($course, $displaysection, $deletesection);
        ...
        //----------------------------------------------start ul 
            ...
            print '<ul class="nav nav-tabs mb-2">';
            //--------------------------------
            ...
            if (empty($course->educationlevel && $course->time && $course->firstrequired && $course->file)) {
                $course->educationlevel =$course->file=$course->firstrequired= $course->time ="Không có";
            }
