...
//Nhien elo 22_07_2019
    public function availability_deadline($text, $additionalclasses = '') {
        ...
        return $this->render_from_template('core/availability_deadline', $data);
    }
    //End Nhien elo 22_07_2019
//29_07_2019 Nhien elo
...
//29_07_2019 Nhien elo
$output .= $this->course_section_cm_deadline($mod, $displayoptions);
//29_07_2019 End Nhien elo
    ...
    //Nhien elo  hoc vien
    public function show_hocvien($searchcriteria) {
        global $CFG;
        require_once($CFG->libdir. '/coursecatlib.php');
        $content = '';
        if (!empty($searchcriteria)) {
            ...
            if (!empty($searchcriteria['search'])) {
                // print search form only if there was a search by search string, otherwise it is confusing
                $content .= $this->box_start('generalbox mdl-align');
                $content .= $this->course_search_form_hocvien($searchcriteria['search']);
                $content .= $this->box_end();
            }
            $displayoptions = array('sort' => array('displayname' => 1));
            // take the current page and number of results per page from query
            ...
            // options 'paginationurl' and 'paginationallowall' are only used in method coursecat_courses()
            $displayoptions['paginationurl'] = new moodle_url('/course/managementstudent.php', $searchcriteria);
            $displayoptions['paginationallowall'] = true; // allow adding link 'View all'
            ...
            $courses = coursecat::search_courses($searchcriteria, $chelper->get_courses_display_options());
            $totalcount = coursecat::search_courses_count($searchcriteria);
            $courseslist = $this->coursecat_courses_elo_quanlyhocvien($chelper, $courses, $totalcount);
            ...
            // just print search form
            $content .= $this->box_start('generalbox mdl-align');
            $content .= $this->course_search_form_hocvien();
            $content .= $this->box_end();
            ...
            $totalcount = 0;
            foreach ($courses_list as $idcourses) {
                if ($idcourses->id == 1) {
                    continue;
                }
                if ($idcourses->visible == 0) {
                    continue;
                }
                if ($idcourses->showcourse == 0) {
                    continue;
                }
                $totalcount++;
            }
            $courseslist = $this->coursecat_courses_elo_quanlyhocvien($chelper, $courses, $totalcount);
            $content .= get_string('totalcoursesearchresults', 'backup', $totalcount);
            $content .= $courseslist;
        }
        return $content;
    }
    //End Nhien elo hoc vien
    
    //Nhien elo giang vien
    public function show_giangvien($searchcriteria) {
        global $CFG;
        require_once($CFG->libdir. '/coursecatlib.php');
        $content = '';
        if (!empty($searchcriteria)) {
            ...
            if (!empty($searchcriteria['search'])) {
                // print search form only if there was a search by search string, otherwise it is confusing
                $content .= $this->box_start('generalbox mdl-align');
                $content .= $this->course_search_form_giangvien($searchcriteria['search']);
                $content .= $this->box_end();
            }
            $displayoptions = array('sort' => array('displayname' => 1));
            // take the current page and number of results per page from query
            ...
            // options 'paginationurl' and 'paginationallowall' are only used in method coursecat_courses()
            $displayoptions['paginationurl'] = new moodle_url('/course/managementteacher.php', $searchcriteria);
            $displayoptions['paginationallowall'] = true; // allow adding link 'View all'
            ...
            $courses = coursecat::search_courses($searchcriteria, $chelper->get_courses_display_options());
            $totalcount = coursecat::search_courses_count($searchcriteria);
            $courseslist = $this->coursecat_courses_elo_quanlygiangvien($chelper, $courses, $totalcount);
            ...
            // just print search form
            $content .= $this->box_start('generalbox mdl-align');
            $content .= $this->course_search_form_giangvien();
            $content .= $this->box_end();
            ...
            // options 'paginationurl' and 'paginationallowall' are only used in method coursecat_courses()
            $displayoptions['paginationurl'] = new moodle_url('/course/managementteacher.php', $searchcriteria);
            $displayoptions['paginationallowall'] = false; // allow adding link 'View all'  
            $class = 'course-search-result';
            foreach ($searchcriteria as $key => $value) {
                if (!empty($value)) {
                    $class .= ' course-search-result-'. $key;
            ...
            $courses = coursecat::search_courses($searchcriteria, $chelper->get_courses_display_options());
            //$totalcount = coursecat::search_courses_count($searchcriteria); // get all course
            $courses_list= get_courses();
            $totalcount = 0;
            foreach ($courses_list as $idcourses) {
                if ($idcourses->visible ==1 && $idcourses->id != 1 && $idcourses->showcourse ==1) {
                    $totalcount++;
                }
            }
            $courseslist = $this->coursecat_courses_elo_quanlygiangvien($chelper, $courses, $totalcount);

            $content .= get_string('availablecourses').': '. $totalcount;
            $content .= $courseslist;
        }
        return $content;
    }
    // End Nhien elo giang vien
