...
defined('MOODLE_INTERNAL') || die();
  ...
  */
function xmldb_block_elo_reminder_users_install() {
    global $DB;

    if ($DB->count_records('block_instances') < 1) {
        // Only add the elo_reminder_users block if it's being installed on an existing site.
    ...
    $page->blocks->add_block('elo_reminder_users', 'content', 0, false, 'my-index', $subpagepattern);
    ...
    // elo_reminder_users block without the admin resetting all of the dashboards.
    //
    // Instead we'll just add the elo_reminder_users block to their dashboards here.
    ...
    foreach ($existingrecords as $existingrecord) {
        ...
        if (isset($seencontexts[$parentcontextid])) {
            ...
            // to add duplicate elo_reminder_users blocks to the same context. This happens
