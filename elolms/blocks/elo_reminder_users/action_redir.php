...
$PAGE->set_url('/blocks/elo_reminder_users/action_redir.php', array('formaction' => $formaction, 'id' => $id));
...
if ($formaction == 'bulkchange.php') {
    ...
    $default = new moodle_url('/blocks/elo_reminder_users/index.php', ['id' => $course->id]);
        ...
        if (has_capability('moodle/course:manageactivities', $context)) {
            ...
            if (isset($plugins[$dataformat])) {
                if ($plugins[$dataformat]->is_enabled()) {
                    ...
                    $columnnames = array(
                        // 'no' => get_string('histories:no', 'block_elo_reminder_users'),
                    ...
                    $offlineusers = new fetcher($currentgroup, $now, $timetoshowteachers, $timetoshowstudents, $PAGE->context,
                            $sitelevel, $PAGE->course->id, $berutype);

                    // if($sitelevel){
                    //     $columnnames['lastsiteaccess'] = get_string('lastsiteaccess', 'block_elo_reminder_users');
                    // }
                    // else{
                    //     $columnnames['lastcourseaccess'] = get_string('lastcourseaccess', 'block_elo_reminder_users');
                    // }
                    ...
                    $rs = $offlineusers->get_users_export(false,false,false);
                    download_as_dataformat('beru_courseid_' . $course->id . '_participants', $dataformat, $columnnames, $rs, 'elo_export_rowcolumns');
                    $rs->close();
                }
            }
        }
