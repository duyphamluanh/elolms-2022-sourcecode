 ...
 */
function theme_ouelofordson_css_tree_post_processor($tree, $theme) {
 ...
 * @return string
 */
function theme_ouelofordson_get_main_scss_content($theme) {
    global $CFG;
    ...
    foreach ($iterator as $pfile) {
        if (!$pfile->isDot()) {
            ...
            if ($filename == $presetname) {
                $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/preset/' . $pfile);
                $presetisset = true;
            }
        }
    }
    if (!$presetisset) {
        $filename .= '.scss';
        if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_ouelofordson', 'preset', 0, '/', $filename))) {
            $scss .= $presetfile->get_content();
        }
        else {
            // Safety fallback - maybe new installs etc.
            $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/preset/Modern Moodle.scss');
        }
    }

    $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/ouelofordson_variables.scss');

    // Page Layout
    if ($theme->settings->pagelayout == 1) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/pagelayout/layout1.scss');
    }
    if ($theme->settings->pagelayout == 2) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/pagelayout/layout2.scss');
    }
    if ($theme->settings->pagelayout == 3) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/pagelayout/layout3.scss');
    }
    if ($theme->settings->pagelayout == 4) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/pagelayout/layout4.scss');
    }
    if ($theme->settings->pagelayout == 5) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/pagelayout/layout5.scss');
    }

    // Section Style
    if ($theme->settings->sectionlayout == 2) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/sectionlayout/sectionstyle2.scss');
    }
    if ($theme->settings->sectionlayout == 3) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/sectionlayout/sectionstyle3.scss');
    }
    if ($theme->settings->sectionlayout == 4) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/sectionlayout/sectionstyle4.scss');
    }
    if ($theme->settings->sectionlayout == 5) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/sectionlayout/sectionstyle5.scss');
    }
    if ($theme->settings->sectionlayout == 6) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/sectionlayout/sectionstyle6.scss');
    }
    if ($theme->settings->sectionlayout == 7) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/sectionlayout/sectionstyle7.scss');
    }
    if ($theme->settings->sectionlayout == 8) {
        $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/sectionlayout/sectionstyle8.scss');
    }

    $scss .= file_get_contents($CFG->dirroot . '/theme/ouelofordson/scss/styles.scss');
 ...
 * @return array
 */
function theme_ouelofordson_get_pre_scss($theme) {
    global $CFG, $PAGE;
    ...
    'blockwidthouelofordson' => ['blockwidthouelofordson'],
    ...
    if(!$loginbg){ // Nhien Nho Xoa Cached ham nay moi thuc thi
            $loginbg = $CFG->wwwroot. '/theme/ouelofordson/pix/login-page-background.jpg';
    }
    if ($PAGE->theme->settings->showcustomlogin == 1) {
        ...
        if (isset($loginbg)) {
            ...
            $prescss .= 'body#page-login-index {background-image: url("' . $loginbg . '") !important; background-size:cover; background-position:top;}';
        }
    }
 ...
 * @return string
 */
function theme_ouelofordson_get_extra_scss($theme) {
