...
class modchooser extends chooser {
    ...
    public function __construct($course, $modules) {
        global $PAGE;
        // This copy of the modchooser is modified for Hillbrook Anglican School theme ouelofordson
        // It contains functionality to output a section of modules defined as "commonly used" in the theme_ouelofordson settings
        $this->course = $course;

        $sections = [];
        $context = context_course::instance($course->id);
        $showonlycustom = get_config('theme_ouelofordson', 'showonlycustomactivities');
        $showallmanager = get_config('theme_ouelofordson', 'showalltomanager');
        ...
        $commonlyused = array();
        $commonlyusedsetting = get_config('theme_ouelofordson', 'commonlyused');
        ...
        if (count($commonlyused)) {
            $sections[] = new chooser_section('commonlyused', new lang_string('modchoosercommonlyusedtitle', 'theme_ouelofordson', $PAGE->theme->settings->modchoosercustomlabel) , array_map(function ($module) use ($context) {
