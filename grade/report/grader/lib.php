...
class grade_report_grader extends grade_report {
    ...
    public function load_final_grades() {
        ...
        $sql = "SELECT g.*
                  FROM {grade_items} gi,
                       {grade_grades} g
                 WHERE g.itemid = gi.id AND gi.courseid = :courseid {$this->userselect}";   //Nhien ham lay diem     
        ...
        foreach ($userids as $userid) {
            foreach ($allgradeitems as $itemid => $gradeitem) {
                $allgradeitems[$itemid]->regrade_final_grades($userid);// 04_11_2019 tính lại điểm các hoạt động gia hạn
            }
        }        
    }
